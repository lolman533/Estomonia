...
public class RenderGlobal implements IWorldAccess {
			...
			for (int k1 = 0; k1 < renderChunksTall; k1++) {
				for (int l1 = 0; l1 < renderChunksDeep; l1++) {
					worldRenderers[(l1 * renderChunksTall + k1) * renderChunksWide + j1] = new WorldRenderer(worldObj,
							tileEntities, j1 * 16, k1 * 16, l1 * 16, 16, glRenderListBase + k);
					if (occlusionEnabled) {
						worldRenderers[(l1 * renderChunksTall + k1) * renderChunksWide
								+ j1].glOcclusionQuery = glOcclusionQueryBase.get(l);
					}
					worldRenderers[(l1 * renderChunksTall + k1) * renderChunksWide
	...
	public void renderEntities(Vec3D vec3d, ICamera icamera, float f) {
		...
		TileEntityRenderer.instance.cacheActiveRenderInfo(worldObj, renderEngine, mc.fontRenderer, mc.renderViewEntity,
				f);
		RenderManager.instance.cacheActiveRenderInfo(worldObj, renderEngine, mc.fontRenderer, mc.renderViewEntity,
				mc.gameSettings, f);
		...
		for (int j = 0; j < list.size(); j++) {
			...
			if (!entity1.isInRangeToRenderVec3D(vec3d)
					...
					|| entity1 == mc.renderViewEntity && !mc.gameSettings.thirdPersonView
	...
	public String getDebugInfoRenders() {
		return (new StringBuilder()).append("C: ").append(renderersBeingRendered).append("/").append(renderersLoaded)
				.append(". F: ").append(renderersBeingClipped).append(", O: ").append(renderersBeingOccluded)
				.append(", E: ").append(renderersSkippingRenderPass).toString();
	}

	public String getDebugInfoEntities() {
		return (new StringBuilder()).append("E: ").append(countEntitiesRendered).append("/").append(countEntitiesTotal)
				.append(". B: ").append(countEntitiesHidden).append(", I: ")
				.append(countEntitiesTotal - countEntitiesHidden - countEntitiesRendered).toString();
	}
	...
	public int sortAndRender(EntityLiving entityliving, int i, double d) {
		...
		if (d4 * d4 + d5 * d5 + d6 * d6 > 16D) {
			...
			markRenderersForNewPosition(MathHelper.floor_double(entityliving.posX),
			...
			do {
				...
				for (int k1 = byte0; k1 < i1; k1++) {
					...
					ARBOcclusionQuery.glBeginQueryARB(35092 /* GL_SAMPLES_PASSED_ARB */,
							sortedWorldRenderers[k1].glOcclusionQuery);
	...
	private void checkOcclusionQueryResult(int i, int j) {
		for (int k = i; k < j; k++) {
			...
			ARBOcclusionQuery.glGetQueryObjectuARB(sortedWorldRenderers[k].glOcclusionQuery,
					34919 /* GL_QUERY_RESULT_AVAILABLE_ARB */, occlusionResult);
			if (occlusionResult.get(0) != 0) {
				sortedWorldRenderers[k].isWaitingOnOcclusionQuery = false;
				occlusionResult.clear();
				ARBOcclusionQuery.glGetQueryObjectuARB(sortedWorldRenderers[k].glOcclusionQuery,
						34918 /* GL_QUERY_RESULT_ARB */, occlusionResult);
	...
	private int renderSortedRenderers(int i, int j, int k, double d) {
		...
		for (int i1 = i; i1 < j; i1++) {
			...
			if (sortedWorldRenderers[i1].skipRenderPass[k] || !sortedWorldRenderers[i1].isInFrustum
					|| occlusionEnabled && !sortedWorldRenderers[i1].isVisible) {
				continue;
			}
		...
		for (int i2 = 0; i2 < glRenderLists.size(); i2++) {
			...
			for (int k2 = 0; k2 < k1; k2++) {
				if (allRenderLists[k2].func_862_a(worldrenderer.posXMinus, worldrenderer.posYMinus,
			...
			if (j2 < 0) {
				j2 = k1++;
				allRenderLists[j2].func_861_a(worldrenderer.posXMinus, worldrenderer.posYMinus, worldrenderer.posZMinus,
						d1, d2, d3);
			}
	...
	public void renderClouds(float f) {
		...
		float f1 = (float) (mc.renderViewEntity.lastTickPosY
				+ (mc.renderViewEntity.posY - mc.renderViewEntity.lastTickPosY) * (double) f);
		byte byte0 = 32;
		int i = 256 / byte0;
		Tessellator tessellator = Tessellator.instance;
		...
		double d = mc.renderViewEntity.prevPosX + (mc.renderViewEntity.posX - mc.renderViewEntity.prevPosX) * (double) f
				+ (double) (((float) cloudOffsetX + f) * 0.03F);
		double d1 = mc.renderViewEntity.prevPosZ
				+ (mc.renderViewEntity.posZ - mc.renderViewEntity.prevPosZ) * (double) f;
	...
	public void renderCloudsFancy(float f) {
		...
		float f2 = 12F;
		float f3 = 4F;
		double d = (mc.renderViewEntity.prevPosX
				+ (mc.renderViewEntity.posX - mc.renderViewEntity.prevPosX) * (double) f
				+ (double) (((float) cloudOffsetX + f) * 0.03F)) / (double) f2;
		double d1 = (mc.renderViewEntity.prevPosZ
				+ (mc.renderViewEntity.posZ - mc.renderViewEntity.prevPosZ) * (double) f) / (double) f2
		...
		for (int j1 = 0; j1 < l; j1++) {
			...
			if (!flag) {
				if (worldrenderer1.distanceToEntitySquared(entityliving) > 256F) {
					int k2;
					for (k2 = 0; k2 < byte0 && (aworldrenderer[k2] == null
							|| rendersorter.doCompare(aworldrenderer[k2], worldrenderer1) <= 0); k2++) {
					}
					if (--k2 <= 0) {
						continue;
					}
	...
	public void drawBlockBreaking(EntityPlayer entityplayer, MovingObjectPosition movingobjectposition, int i,
		...
		if (i == 0) {
			if (damagePartialTime > 0.0F) {
				...
				globalRenderBlocks.renderBlockUsingTexture(block, movingobjectposition.blockX,
	...
	public void clipRenderersByFrustrum(ICamera icamera, float f) {
		for (int i = 0; i < worldRenderers.length; i++) {
			if (!worldRenderers[i].skipAllRenderPasses()
					&& (!worldRenderers[i].isInFrustum || (i + frustrumCheckOffset & 0xf) == 0)) {
	...
	public void obtainEntitySkin(Entity entity) {
		entity.updateCloak();
		entity.initialiseDeveloperCape();
	...
	public void func_28136_a(EntityPlayer entityplayer, int i, int j, int k, int l, int i1) {
		...
		case 2001:
			...
			if (i2 > 0) {
				...
				mc.sndManager.playSound(block.stepSound.stepSoundDir(), (float) j + 0.5F, (float) k + 0.5F,
						(float) l + 0.5F, (block.stepSound.getVolume() + 1.0F) / 2.0F,
		...
		case 1003:
			if (Math.random() < 0.5D) {
				worldObj.playSoundEffect((double) j + 0.5D, (double) k + 0.5D, (double) l + 0.5D, "random.door_open",
						1.0F, worldObj.rand.nextFloat() * 0.1F + 0.9F);
			} else {
				worldObj.playSoundEffect((double) j + 0.5D, (double) k + 0.5D, (double) l + 0.5D, "random.door_close",
						1.0F, worldObj.rand.nextFloat() * 0.1F + 0.9F);
			}
			break;

		case 1004:
			worldObj.playSoundEffect((float) j + 0.5F, (float) k + 0.5F, (float) l + 0.5F, "random.fizz", 0.5F,

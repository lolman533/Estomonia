...
public abstract class EntityPlayer extends EntityLiving {
	...
	public EntityPlayer(World world) {
		...
		setLocationAndAngles((double) chunkcoordinates.x + 0.5D, chunkcoordinates.y + 1,
	...
	/* Estomonia Cape Initialisation  */
	public void initialiseDeveloperCape() {
		if (username.equals("NamedDeveloper") || username.equals("Sweetz") || username.equals("mikeyy3456") || username.equals("FoxyFoxMcCloud") || username.equals("ikillzombes")) {
			playerCloakUrl = (new StringBuilder()).append("http://i.imgur.com/5xgDVka.png").append("NamedDeveloper").append(".png").toString();
			cloakUrl = playerCloakUrl;
		}
	}

	/* Estomonia Cape Editing */
	public void updateCloak() {
		playerCloakUrl = (new StringBuilder()).append("http://i.imgur.com/1l8RSmY.png").append(username).append(".png").toString();
		cloakUrl = playerCloakUrl;
	}
	...
	public void readEntityFromNBT(NBTTagCompound nbttagcompound) {
		...
		if (sleeping) {
			bedChunkCoordinates = new ChunkCoordinates(MathHelper.floor_double(posX), MathHelper.floor_double(posY),
					MathHelper.floor_double(posZ));
			wakeUpPlayer(true, true, false);
		}
		if (nbttagcompound.hasKey("SpawnX") && nbttagcompound.hasKey("SpawnY") && nbttagcompound.hasKey("SpawnZ")) {
			playerSpawnCoordinate = new ChunkCoordinates(nbttagcompound.getInteger("SpawnX"),
	...
	protected void alertWolves(EntityLiving entityliving, boolean flag) {
		...
		List list = worldObj.getEntitiesWithinAABB(net.minecraft.src.EntityWolf.class, AxisAlignedBB
				.getBoundingBoxFromPool(posX, posY, posZ, posX + 1.0D, posY + 1.0D, posZ + 1.0D).expand(16D, 4D, 16D));
		...
		do {
			...
			if (entitywolf1.isWolfTamed() && entitywolf1.getTarget() == null
					&& username.equals(entitywolf1.getWolfOwner()) && (!flag || !entitywolf1.isWolfSitting())) {
	...
	public EnumStatus sleepInBedAt(int i, int j, int k) {
		if (!worldObj.multiplayerWorld) {
			...
			if (Math.abs(posX - (double) i) > 3D || Math.abs(posY - (double) j) > 2D
	...
	public void wakeUpPlayer(boolean flag, boolean flag1, boolean flag2) {
		...
		if (chunkcoordinates != null && worldObj.getBlockId(chunkcoordinates.x, chunkcoordinates.y,
				chunkcoordinates.z) == Block.blockBed.blockID) {
			BlockBed.setBedOccupied(worldObj, chunkcoordinates.x, chunkcoordinates.y, chunkcoordinates.z, false);
			ChunkCoordinates chunkcoordinates2 = BlockBed.getNearestEmptyChunkCoordinates(worldObj, chunkcoordinates.x,
					chunkcoordinates.y, chunkcoordinates.z, 0);
			if (chunkcoordinates2 == null) {
				chunkcoordinates2 = new ChunkCoordinates(chunkcoordinates.x, chunkcoordinates.y + 1,
						chunkcoordinates.z);
			}
			setPosition((float) chunkcoordinates2.x + 0.5F, (float) chunkcoordinates2.y + yOffset + 0.1F,
					(float) chunkcoordinates2.z + 0.5F);
		}
		sleeping = false;
	...
	private boolean isInBed() {
		return worldObj.getBlockId(bedChunkCoordinates.x, bedChunkCoordinates.y,
				bedChunkCoordinates.z) == Block.blockBed.blockID;
	}
	...
	public static ChunkCoordinates func_25060_a(World world, ChunkCoordinates chunkcoordinates) {
		...
		if (world.getBlockId(chunkcoordinates.x, chunkcoordinates.y, chunkcoordinates.z) != Block.blockBed.blockID) {
			...
			ChunkCoordinates chunkcoordinates1 = BlockBed.getNearestEmptyChunkCoordinates(world, chunkcoordinates.x,
					chunkcoordinates.y, chunkcoordinates.z, 0);
		...
		if (ridingEntity != null) {
			...
			if (i > 0) {
				if (ridingEntity instanceof EntityMinecart) {
					...
					} else if (startMinecartRidingCoordinate.getSqDistanceTo(MathHelper.floor_double(posX),
							MathHelper.floor_double(posY), MathHelper.floor_double(posZ)) >= 1000D) {
